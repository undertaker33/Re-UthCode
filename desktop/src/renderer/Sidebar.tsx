...
function ProjectEntry({ project, props }: { project: ProjectState; props: SidebarProps }) {
  ...
  return <li className={`project-item${active ? " is-active" : ""}`}>
    <div className="project-line">
      <button type="button" className="icon-button disclosure" title={`${expanded ? "Collapse" : "Expand"} ${project.alias}`} aria-label={`${expanded ? "Collapse" : "Expand"} ${project.alias}`} aria-expanded={expanded} onClick={() => setExpanded((value) => !value)}><UiIcon name="chevron" /></button>
      ...
      <div className="row-actions"><button type="button" className="icon-button" title={`Rename ${project.alias}`} aria-label={`Rename ${project.alias}`} onClick={() => setEditing(true)}><UiIcon name="edit" /></button><button type="button" className="icon-button" title={project.pinned ? `Unpin ${project.alias}` : `Pin ${project.alias}`} aria-label={project.pinned ? `Unpin ${project.alias}` : `Pin ${project.alias}`} onClick={() => props.onTogglePin(project)}><UiIcon name="pin" /></button><button type="button" className="icon-button" title={`Open ${project.alias} in Explorer`} aria-label={`Open ${project.alias} in Explorer`} onClick={() => props.onOpenExplorer(project)}><UiIcon name="external" /></button><button type="button" className="icon-button danger" title={`Remove ${project.alias}`} aria-label={`Remove ${project.alias}`} onClick={() => setConfirming(true)}><UiIcon name="trash" /></button></div>
    </div>
...
function PinnedSessions({ entries, props }: { entries: Array<{ project: ProjectState; session: SessionSummary }>; props: SidebarProps }) {
  if (!entries.length) return null;
  return <section className="nav-group pinned-sessions" aria-labelledby="nav-pinned-sessions"><h2 id="nav-pinned-sessions">Pinned sessions</h2><ul>{entries.map(({ project, session }) => <li className="session-item" key={`${project.projectKey}:${session.session_id}`}><button type="button" className={`recent-line${session.session_id === props.selectedSessionId ? " is-selected" : ""}`} onClick={() => props.onResumeSession(project, session.session_id)}><span className="session-dot" /><span>{sessionLabel(session)}</span><small>{project.alias}</small></button><button type="button" className="icon-button session-pin" title={`Unpin session ${sessionLabel(session)}`} aria-label={`Unpin session ${sessionLabel(session)}`} onClick={() => props.onToggleSessionPin(project, session)}><UiIcon name="pin" /></button></li>)}</ul></section>;
}
