...
export function CustomSelect({ value, options, onChange, label, disabled = false, id, onOpen }: CustomSelectProps) {
  ...
  const keyDown = (event: React.KeyboardEvent) => {
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowUp") { event.preventDefault(); event.stopPropagation(); if (!open) { onOpen?.(); setOpen(true); } else move(event.key === "ArrowDown" ? 1 : -1); return; }
    if (open && (event.key === "Home" || event.key === "End")) { event.preventDefault(); event.stopPropagation(); edge(event.key === "End"); return; }
    if (event.key === "Enter" || event.key === " ") { event.preventDefault(); event.stopPropagation(); if (open) choose(active); else { onOpen?.(); setOpen(true); } }
  };
  ...
  return <div className="custom-select" ref={root} onKeyDown={keyDown} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setOpen(false); }}>
    <button ref={trigger} id={id} type="button" className="custom-select__trigger" title={label} aria-label={label} aria-haspopup="listbox" aria-expanded={open} aria-controls={listId} disabled={disabled} onClick={() => { if (!open) onOpen?.(); setOpen(!open); }}>{selected?.label ?? value}<span aria-hidden="true">⌄</span></button>
