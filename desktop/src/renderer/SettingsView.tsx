...
export function SettingsView({ state, onBack, onSave, onThemeChange }: SettingsViewProps) {
  ...
  return (
    <section className="settings-view" aria-label="Settings">
      <aside className="settings-nav" aria-label="Settings sections"><button type="button" className="settings-view__back" onClick={onBack}>← Back to chat</button><div><p className="eyebrow">Settings</p><h1>Preferences</h1></div><nav aria-label="Visible settings groups"><a href="#settings-providers">Providers</a><a href="#settings-models">Models</a><a href="#settings-defaults">Defaults</a><a href="#settings-interface">Interface</a><a href="#settings-about">About</a></nav></aside>
      <div className="settings-content"><header><div><p className="eyebrow">Workspace configuration</p><h1>Settings</h1><p>Configure the providers, models, defaults, and local interface preferences used by UthCode.</p></div><span className={`settings-runtime settings-runtime--${state.runtimeState}`}>{state.runtimeState.replace(/_/gu, " ")}</span></header>
      {state.settingsError && <p className="settings-view__error" role="alert">{state.settingsError}</p>}
      <section className="settings-section" id="settings-providers"><h2>Providers</h2><p className="settings-section__hint">Provider profiles used by the current model catalog.</p>{providers.map(([id, profile]) => <div className="settings-row" key={id}><label htmlFor={`provider-id-${id}`}>Provider ID</label><div><div className="settings-inline"><input id={`provider-id-${id}`} aria-label={`${id} provider id`} defaultValue={id} onBlur={(event) => commitProviderId(id, event.target.value, event.currentTarget)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} /><select id={`provider-kind-${id}`} aria-label={`${id} provider kind`} value={String(profile.kind ?? "fake")} onChange={(event) => setDraft((current) => ({ ...current, providers: updateRecord(current.providers, id, "kind", event.target.value) }))}><option value="fake">fake</option><option value="anthropic">anthropic</option><option value="openai_responses">openai_responses</option><option value="openai_compat">openai_compat</option></select><input aria-label={`${id} base URL`} placeholder="Base URL" value={String(profile.base_url ?? "")} onChange={(event) => setDraft((current) => ({ ...current, providers: updateRecord(current.providers, id, "base_url", event.target.value || null) }))} /><button type="button" aria-label={`Remove provider ${id}`} onClick={() => removeProvider(id)}>Remove</button></div><div className="settings-secret-line"><span className="settings-row__secret">{profile.api_key_configured === true ? "configured" : "not configured"}</span><input aria-label={`${id} API key replacement`} type="password" autoComplete="new-password" placeholder={profile.api_key_configured === true ? "Replace API key" : "API key"} value={apiKeys[id] ?? ""} onChange={(event) => { setApiKeys((current) => ({ ...current, [id]: event.target.value })); setTouchedKeys((current) => ({ ...current, [id]: true })); }} />{profile.api_key_configured === true && <button type="button" aria-label={`Clear API key for provider ${id}`} onClick={() => { setApiKeys((current) => ({ ...current, [id]: "" })); setTouchedKeys((current) => ({ ...current, [id]: true })); }}>Clear key</button>}</div></div></div>)}<button type="button" className="row-add" onClick={addProvider}>＋ Add provider</button></section>
      <section className="settings-section" id="settings-models"><h2>Models</h2><p className="settings-section__hint">Model references available to the Application.</p>{models.map(([ref, profile]) => <div className="settings-row settings-row--model" key={ref}><label htmlFor={`model-ref-${ref}`}>Model reference</label><div className="settings-model-fields"><input id={`model-ref-${ref}`} aria-label={`${ref} model ref`} defaultValue={ref} onBlur={(event) => commitModelRef(ref, event.target.value, event.currentTarget)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} /><input aria-label={`${ref} remote model`} placeholder="Remote model ID" value={String(profile.remote_id ?? "")} onChange={(event) => setDraft((current) => ({ ...current, models: updateRecord(current.models, ref, "remote_id", event.target.value) }))} /><select aria-label={`${ref} provider`} value={String(profile.provider_profile_id ?? "")} onChange={(event) => setDraft((current) => ({ ...current, models: updateRecord(current.models, ref, "provider_profile_id", event.target.value) }))}>{providers.map(([id]) => <option value={id} key={id}>{id}</option>)}</select><input aria-label={`${ref} display name`} placeholder="Display name" value={String(profile.display_name ?? "")} onChange={(event) => setDraft((current) => ({ ...current, models: updateRecord(current.models, ref, "display_name", event.target.value) }))} /><input aria-label={`${ref} context window`} type="number" min="1" step="1" placeholder="Context window" value={profile.context_window == null ? "" : String(profile.context_window)} onChange={(event) => setDraft((current) => ({ ...current, models: updateRecord(current.models, ref, "context_window", parseOptionalPositiveInteger(event.target.value)) }))} /><input aria-label={`${ref} max output tokens`} type="number" min="1" step="1" placeholder="Max output tokens" value={profile.max_output_tokens == null ? "" : String(profile.max_output_tokens)} onChange={(event) => setDraft((current) => ({ ...current, models: updateRecord(current.models, ref, "max_output_tokens", parseOptionalPositiveInteger(event.target.value)) }))} /><select aria-label={`${ref} reasoning effort`} value={String(profile.reasoning_effort ?? "none")} onChange={(event) => setDraft((current) => ({ ...current, models: updateRecord(current.models, ref, "reasoning_effort", event.target.value) }))}>{["none", "minimal", "low", "medium", "high", "xhigh", "max"].map((effort) => <option value={effort} key={effort}>{effort}</option>)}</select><button type="button" aria-label={`Remove model ${ref}`} onClick={() => removeModel(ref)}>Remove</button></div></div>)}<button type="button" className="row-add" onClick={addModel}>＋ Add model</button></section>
      <section className="settings-section" id="settings-defaults"><h2>Permissions</h2><p className="settings-section__hint">Default permission mode for new Runs.</p><div className="settings-row"><label htmlFor="default-permission">Default mode</label><select id="default-permission" value={draft.default_permission_mode ?? "default"} onChange={(event) => setDraft((current) => ({ ...current, default_permission_mode: event.target.value === "auto" ? "auto" : "default" }))}><option value="default">default</option><option value="auto">auto</option></select></div><div className="settings-row"><label className="settings-row__label" htmlFor="default-model">Default model</label><select id="default-model" value={draft.default_model ?? ""} onChange={(event) => setDraft((current) => ({ ...current, default_model: event.target.value }))}><option value="">Select a model</option>{models.map(([ref]) => <option value={ref} key={ref}>{ref}</option>)}</select></div></section>
      <section className="settings-section" id="settings-interface"><h2>Interface</h2><p className="settings-section__hint">Appearance preferences are stored on this Desktop installation.</p><div className="settings-row"><label htmlFor="theme">Theme</label><select id="theme" value={state.theme} onChange={(event) => onThemeChange(event.target.value as ThemePreference)}><option value="system">system</option><option value="dark">dark</option><option value="light">light</option></select></div></section>
