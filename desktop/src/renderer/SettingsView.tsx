...
export function SettingsView({ state, onBack, onSave, onThemeChange }: SettingsViewProps) {
  ...
  return (
    <section className="settings-view" aria-label="Settings">
      ...
      <section className="settings-section" id="settings-providers"><h2>Providers</h2><p className="settings-section__hint">Connect the service that runs your models.</p>{providers.map(([id, profile]) => <div className="settings-profile" key={id}>
        ...
        <div className="settings-row"><label htmlFor={`provider-key-${id}`}>API Key</label><div className="settings-secret-line"><span className="settings-row__secret">{profile.api_key_configured === true ? "configured" : "not configured"}</span><input id={`provider-key-${id}`} aria-label={`${id} API key replacement`} type="password" autoComplete="new-password" placeholder={profile.api_key_configured === true ? "Replace API key" : "API key"} value={apiKeys[id] ?? ""} onChange={(event) => { setApiKeys((current) => ({ ...current, [id]: event.target.value })); setTouchedKeys((current) => ({ ...current, [id]: true })); }} />{profile.api_key_configured === true && <button type="button" aria-label={`Clear API key for provider ${id}`} onClick={() => { setApiKeys((current) => ({ ...current, [id]: "" })); setTouchedKeys((current) => ({ ...current, [id]: true })); }}>Clear key</button>}</div></div>
      ...
      </div>)}<button type="button" className="row-add" onClick={addProvider}>＋ Add provider</button></section>
      <section className="settings-section" id="settings-models"><h2>Models</h2><p className="settings-section__hint">Use the model name expected by your provider.</p>{models.map(([ref, profile]) => <div className="settings-profile" key={ref}>
      ...
      </div>)}<button type="button" className="row-add" onClick={addModel}>＋ Add model</button></section>
