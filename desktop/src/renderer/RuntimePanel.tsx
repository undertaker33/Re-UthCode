...
function usageLabel(usage: Record<string, unknown> | undefined, unavailable: string): string {
  ...
  return (
    <aside className={`runtime-panel runtime-panel--${state.panelMode}`} aria-label={t("runtimeInformation")} aria-hidden={state.panelMode === "hidden" || undefined}>
      ...
      <dl className="runtime-facts">
        ...
        <div><dt>{t("context")}</dt><dd>{usageLabel(state.run?.usage, t("unavailable"))}</dd></div>
