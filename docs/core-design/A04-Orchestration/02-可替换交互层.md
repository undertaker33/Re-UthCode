...
当前 Electron Desktop 通过 `preload -> main -> python-runtime -> interfaces/desktop/bridge.py` 调用同一 Application。Renderer 接收安全 DTO 与事件，不持有 Provider SDK、Tool Executor 或 Session 文件。

Desktop 的会话导航展示了“界面状态”和“运行状态”的区别：用户切到会话 B 时，会话 A 的 Turn 可以继续在后台运行。Bridge 保存各 Session 对应的 Application/Run，并给事件附上 Session/Project 身份；Renderer 根据身份更新对应的显示缓存。再次选择 A 可以重新激活已有 runtime。导航不等于取消，进程重启也不会恢复未结束的 Turn 或交互等待。

聊天历史同样有两条不同职责的路径。`history.page` 默认读取最近 30 个完整交互单元，上翻再取更早页；`session.resume` 独立准备完整运行时，冷 Session 准备完成前禁止普通发送。少显示历史只影响阅读，不减少模型上下文。持久压缩提示从 Timeline 恢复，并按实际 Transcript 提交位置插入；Renderer 的实时缓存不成为另一份历史权威。

手动 `/compact` 由 Session 所属后台任务执行，启动响应只返回操作身份，后续通知报告进度和终态。切换会话不会取消压缩，显式取消通过 Session/操作身份交给 Application。界面区分 completed、no_change、cancelled、failed，并显示是否已有有效提交；是否压缩成功仍由实际普通 Working Context 的缩减决定，不能由动画、Provider usage 或用户点击动作推定。

为了让这些投影保持一致，Renderer 的 `state.ts` 集中持有 reducer，`useRuntimeLifecycle` 集中管理 runtime generation、请求归属、过期响应拦截和终态收敛。它们解决的是 Interface 请求与显示生命周期，不替代 Agent Loop 对 `RunState` 的唯一写入权。Current Context 与 Last Provider Request Usage 也保持独立，上一请求的 token 数不能直接替换当前工作上下文测量。

Settings、布局和 Markdown 交互则属于可替换的界面实现。Provider/Model 编辑共用一个弹窗根及焦点生命周期，配置写入仍经过 Configuration Application；API key 的按需显示值只留在编辑器局部状态。侧栏和 Runtime 面板宽度可以保存为 preference，Focus Mode 只临时隐藏面板并在退出时恢复。代码块复制保留原文，历史前插保留阅读位置。这些变化改善交互体验，不改变权限、Tool FIFO、Context 或 Turn 的含义。

## 可替换的真正含义

“可替换 Interface”不是说所有界面行为完全一样。

TUI 可以有命令候选、模型 Picker 和实时流；`exec` 可以只有 stdin/stdout；当前 Windows Desktop 使用按钮和面板。它们的交互体验当然不同。
...
当前代码事实可参考：[A04 Orchestration Context](../../context/A04-Orchestration/Orchestration-Context.md) 、[TUI Context](../../context/TUI/README.md) 与 [GUI Context](../../context/GUI/GUI-Context.md)。
