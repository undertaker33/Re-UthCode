...
Desktop 对手动压缩提供异步操作适配：Bridge 先返回所属 Session 和操作身份，再发布进度及终态，用户可以取消或切到其他会话。是否取消与是否已有有效提交分别显示；后续 epoch 中断不会撤销先前 checkpoint。聊天中的完成提示从持久 Timeline 的提交位置投影，界面不把提示写成模型消息，也不负责恢复 Provider 请求。
