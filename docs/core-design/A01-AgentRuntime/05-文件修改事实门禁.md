...
`ApplyPatch` 把同一事实门禁扩展到一份 Codex patch：先解析完整 patch，拒绝危险路径、重复目标、缺失源文件、未读取或已变化的目标，并在任何目标提交前完成全计划预检，因此预检失败保持零变更。Permission 通过后，执行阶段在每个目标前再次核对版本并以单目标原子替换提交；移动会分别绑定源和目标。若后续提交失败，工具返回已提交、失败和未执行集合，磁盘状态以已发生的部分提交为准，不伪造全局回滚。
