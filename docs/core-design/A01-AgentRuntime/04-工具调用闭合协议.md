...
注册顺序同样不是装饰。活动 Turn 启动时捕获工具定义集合及其顺序，每次 iteration 再依据当前 Behavior Mode 过滤 Provider 可见视图。批准 Plan 后，同一 Turn 从 PLAN 切换为 DEFAULT，后续请求会开放执行模式工具并隐藏 `ProposePlan`；这改变的是模式视图，不是重新注册工具或加载新的定义。后续配置变化不会改写已捕获的定义集合。

当前正式 Integration 集合包含文件、文档、图片、进程、`ApplyPatch`、`GitWorkspace` 和 `WebFetch`；用户级 Tavily 搜索配置有效时才加入 `WebSearch`。`ApplyPatch` 的写入能力在 PLAN 中隐藏，Git 查询、公开 Fetch 和搜索按只读 planning access 进入 PLAN 视图。大正文由 Application 按 inline/ref 策略处理，模型后续通过当前 Session 的 `ToolResultRead` opaque ref 有界续读；续读不会递归物化新的 Tool Result。
