...
| F03 | Context 冻结收口、工程收敛与 Desktop 体验优化 | `docs/work/F03-Context冻结收口与工程收敛及Desktop体验优化/` | W01～W07 Feedback 已齐全；W07 已完成 fixture 修复、全量 Python/Context 定向 measurement、Desktop typecheck/npm test 与 Forge API 隔离 package 证据，标准 npm package 仍因既有 `desktop/out/UthCode-win32-x64/resources/app.asar` EBUSY 未通过，真实 dev identity/event/terminal 完整链、native pointer/Windows zoom、真实 Provider、干净 Windows 和人工视觉仍未验证；Checklist 仍有未勾项，保持 `not_implemented` |
