...
会话名称优先显示手动标题，否则使用首条用户消息预览；没有可用标题和预览时才回退到 Session ID。长名称会省略显示，悬停可查看会话信息；侧栏仅纵向滚动，右侧菜单仍可操作。
...
压缩时，聊天区会显示带动画的单行进度；成功后显示“上下文已压缩”，保留在压缩发生的位置，切换会话或重启后仍可在对应历史页查看。更新前未记录发生位置的压缩无法还原为准确的历史提示。侧栏勾号表示回复或压缩完成后尚未查看；窗口获得焦点并显示聊天尾部后清除，历史提示继续保留。项目整行可展开或收起，右侧菜单独立操作；Runtime 面板提供隐藏、浮动和停靠三个图标按钮。

宽屏 Desktop 的侧栏与 Runtime 面板分隔条支持拖拽和键盘调整，调整后的宽度会保存；窄屏下 Runtime 使用浮层。Focus Mode 临时隐藏侧栏与 Runtime，退出后恢复原来的布局和宽度。用户向上阅读时，流式回复不会强制滚回底部；点击新消息入口可回到底部并继续跟随回复。

聊天支持标题、列表、表格、引用、链接和代码块等 Markdown。代码块显示语言标签并提供复制按钮，复制保留原有换行、空行和空格。原始 HTML 作为文本显示；普通链接仅支持 HTTP、HTTPS 和 mailto，本地文件产物使用受控的 `artifact:` 引用，仍需通过路径与授权检查。侧栏菜单也可复制 Session ID。

### 附件与文件查看

点击输入区的加号、粘贴或拖拽添加附件。文本输入区右键菜单提供剪切、复制、粘贴、全选，也可使用键盘快捷键。图片缩略图显示在输入框文字上方，发送后显示在用户消息正文上方；缩略图完整保留图片比例，不裁切，不显示图片名称、类型和大小。只有附件而没有文字也可以发送。发送被拒绝时草稿保留，输入区附近说明原因和处理方式；图片需要选择已配置图片能力的视觉模型，不能只给不支持视觉的模型打开能力开关。

双击图片在独立弹窗中查看，滚轮或键盘加减号缩放，拖动平移，按 `0` 恢复适应窗口，按 `Escape` 关闭。查看图片不会在聊天正文中展开大图，也不会改变原始文件。

文件卡片用图标、颜色和扩展名区分类型。Word、Excel、PowerPoint 和 PDF 双击后由系统默认应用打开；系统没有关联程序时，按提示先配置默认应用。Markdown、代码和可识别的纯文本双击后进入右侧只读画布，Markdown 可切换渲染与源码，代码显示行号和语法高亮。画布可调整宽度，关闭后恢复原来的右侧布局；超限文本会标明截断，可改用系统应用查看完整文件。画布不编辑或保存文件。

右键文件卡片，或通过键盘菜单键/`Shift+F10` 打开文件操作菜单，可按文件类型预览、系统打开、定位或复制路径。未发送的附件可通过卡片上的“×”或文件菜单移除，包括粘贴的图片；已发送附件不提供移除操作，保留在 Session 中。可执行文件不提供预览或系统打开，可定位或复制路径，未发送时也可移除草稿。工作目录外的产物仍需通过系统文件选择器授权，模型提供路径不等于获得授权。
