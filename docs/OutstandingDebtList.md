...
| **T09 Prompt / Context Engineering** | 探索并实现真实 Context Window / max input limit 获取、可查询 Provider metadata、官方模型 bundled metadata、OpenAI-compatible / Local 显式配置、统一 Model Limits contract，以及真实窗口与 UthCode Working Budget 的解析；正式解决 T09 固定 258K 阶段不保证真实窗口小于 258K 的模型在长上下文下预算安全的问题 | 原 T09-1 工作包已移除；待重新规划工作包并完成实现、Checklist 与 Feedback 后删除本条 |
| **T09 Prompt / Context Engineering** | 为现有有界 `ContextCompactor` 接通正式 tool-free summarizer use case，收口异步 Provider 请求、模型选择、取消、失败和最多一次 overflow retry；在此之前 `/compact` 与 overflow compaction 均只会返回 `summarizer_unavailable` | 原 T09-1 工作包已移除；待重新规划工作包并完成实现、Checklist 与 Feedback 后删除本条 |
| **T09 Prompt / Context Engineering** | 完成 small-window / large-window adaptation，并基于 Prefix Cache、token、success Eval 调优 Operating Profile、Working Set、Compaction trigger、recent tail 与 safety headroom | 原 T09-1 工作包已移除；待重新规划工作包并完成实现、Checklist 与 Feedback 后删除本条 |
