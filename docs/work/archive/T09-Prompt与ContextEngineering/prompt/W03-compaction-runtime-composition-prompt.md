...
7. Runtime 每次请求统一经 Compiler；Core/Application 分字段形成 Instruction Plane、Conversation Plane 与 Tool System 唯一提供的 `GenerationRequest.tools`，Integration 只做原生协议映射；Tool Schema 不复制进 Prompt，ordinary history 不能进入 Instruction Plane，overflow 仅一次最后保护。
