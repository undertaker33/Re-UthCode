...
以用户实际派发本 Prompt 时的当前仓库状态为实施起点，不要求 HEAD 等于任何固定 SHA，也不要求 checkout 历史 Commit。完成下列必读项后，必须重新核对当前真实 `src/ + tests/` 与冻结任务定义；只有源码实质变化已经使产品语义、架构边界或 T01 完成范围失效时，才停止相关范围并按 Feedback 规则报告。普通后续 Commit、任务包修订或文档更新不构成基线冲突。
