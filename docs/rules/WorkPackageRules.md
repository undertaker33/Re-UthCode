...
每次创建或重新拆分任务包时，拆分 Agent 必须读取 `docs/OutstandingDebtList.md`，并在当前任务包中设置“能力欠账”章节。该章节只记录：当前 `TXX` 因为后置能力尚未实现而刻意没有继续设计或实施的部分。

- 新出现的真实欠账必须写明来源 `TXX`、欠账需求以及回补前置或触发条件，并同步加入清单；
