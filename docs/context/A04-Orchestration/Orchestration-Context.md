...
- `[FACT]` `ApplicationContextService` 是正式请求组合入口：Instruction Plane、Conversation Plane、预算来源和 `GenerationRequest.tools` 进入同一 Provider-independent DTO；Integration 不重新编译 Context。预算参数和 Gate 规则统一见 [A03 Context 预算与诊断](../A03-State/State-Context.md#context-预算与诊断)。
...
- `[FACT]` 附件导入、预览、移除和提交状态更新沿同一 Application Session 边界组合；`AttachmentService` 不建立通用 Artifact Store，Provider 只在真实请求组装边界通过 Session-owned ref 读取原始字节。Session writer 在正式 open/close 生命周期重试提交 metadata 并清理无引用临时/派生文件，恢复不重做已提交 Turn。
